// First_video_synch.cpp: определяет точку входа для консольного приложения.
//
...
// Основные функции синхронизации видеопотоков


// Гистограммы
void synchronization_hists(int frames_count1, int frames_count2, MatND* video_1, MatND* video_2, int &index1, int &index2)
{
	// Инициализация переменных	
	int min_size = (int)(0.5 * (double)frames_count1);
	double min_comp = frames_count1 * 10;
	double current_comp, bufer_comp;
	index1 = 0;
	index2 = 0;
	...
	for (int i = min_size; i < frames_count1; i++)
	{
		...
		for (int k = 0; k < i; k++)
		{
			// Находим значение схожести
			bufer_comp = compareHist(video_1[k + frames_count1 - i], video_2[k], CV_COMP_HELLINGER);
			// Плюсуем к итоговому значению по определенной метрике
			// Евкдлидова
			//current_comp += pow((bufer_comp), 2);
			// L1
			current_comp += bufer_comp;
		}
	...
	for (int i = 0; i < frames_count2 - frames_count1 + 1; i++)
	{
		...
		{
			// Находим значение схожести
			bufer_comp = compareHist(video_1[k], video_2[i + k], CV_COMP_HELLINGER);
			// Плюсуем к итоговому значению по определенной метрике
			// Евкдлидова
			//current_comp += pow((bufer_comp), 2);
			// L1
	...
	for (int i = frames_count1 - 1; i >= min_size; i--)
	{
		...
		{
			// Находим значение схожести
			bufer_comp = compareHist(video_1[k], video_2[frames_count2 - i + k], CV_COMP_HELLINGER);
			// Плюсуем к итоговому значению по определенной метрике
			// Евкдлидова
			//current_comp += pow((bufer_comp), 2);
			// L1
...
// Матрицы переходов
void synchronization_glcms(int frames_count1, int frames_count2, double*** video_1, double*** video_2, int &index1, int &index2)
{
	// Инициализация переменных	
	int min_size = (int)(0.6 * (double)frames_count1);
	double min_comp = frames_count1 * 10;
	double current_comp, bufer_comp;
	index1 = 0;
	index2 = 0;
	...
	for (int i = min_size; i < frames_count1; i++)
	{
		...
		{
			// Находим значение схожести
			bufer_comp = glcmDistance(video_1[k + frames_count1 - i], video_2[k]);
			// Плюсуем к итоговому значению по определенной метрике
			// Евкдлидова
			//current_comp += pow((bufer_comp), 2);
			// L1
	...
	for (int i = 0; i < frames_count2 - frames_count1 + 1; i++)
	{
		...
		{
			// Находим значение схожести
			bufer_comp = glcmDistance(video_1[k], video_2[i + k]);
			// Плюсуем к итоговому значению по определенной метрике
			// Евкдлидова
			//current_comp += pow((bufer_comp), 2);
			// L1
	...
	for (int i = frames_count1 - 1; i >= min_size; i--)
	{
		...
		{
			// Находим значение схожести
			bufer_comp = glcmDistance(video_1[k], video_2[frames_count2 - i + k]);
			// Плюсуем к итоговому значению по определенной метрике
			// Евкдлидова
			//current_comp += pow((bufer_comp), 2);
			// L1
...
int main()
{
	...
	// Объявление переменных на каждый из варианто
	// Гистограммы
	int bins = 4;
	...
	double*** video_2_glcms = new double**[frames_count2];
	...
	*/
	...
	// Гистограммы
	synchronization_hists(frames_count1 - 1, frames_count2 - 1, indices1, indices2, start1, start2);

	// Матрицы переходов
	//synchronization_glcms(frames_count1 - 1, frames_count2 - 1, video_1_glcms, video_2_glcms, start1, start2);
	finish_time = clock();
